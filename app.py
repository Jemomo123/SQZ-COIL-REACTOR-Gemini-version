...
# --- THE FAILOVER CHAIN ---
# Initializing the exchanges defined in your fallback protocol
EXCHANGE_CHAIN = [
...
# DATA ENGINE WITH AUTOMATIC FAILOVER
def fetch_and_process(exchange_info, symbol, tf):
    """Attempt to fetch data and process Jeremiah Edge logic for a specific exchange."""
    ex_obj = exchange_info["obj"]
    name = exchange_info["name"]
    
    # Handle MEXC/GateIO specific symbol naming if necessary (standardizing to USDT)
...
def get_timeframe_signal(symbol, tf):
    """Iterates through the exchange chain until a successful fetch occurs."""
    for exchange_info in EXCHANGE_CHAIN:
        try:
            return fetch_and_process(exchange_info, symbol, tf)
        except Exception as e:
            logger.warning(f"{exchange_info['name']} FAIL for {symbol} {tf}. Trying next...")
            continue # Try next exchange in chain
...
with col2:
    if outages:
        st.markdown(f"<p class='status-dim'>⚠️ Total Outages: {', '.join(set(outages))}</p>", unsafe_allow_html=True)
